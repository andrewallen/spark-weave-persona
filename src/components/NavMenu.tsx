...
const NavMenu = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'py-4 bg-navy/80 backdrop-blur-md' : 'py-6'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-8">
          {menuItems.map(item => (
            <a key={item.id} href={`#${item.id}`} className="nav-item interactive">
              {item.label}
            </a>
          ))}
